...
describe('avatars', function() {
	...
	describe('resizeImage', function() {
		...
		it('should resize and crop an image to the specified size', function(done) {
			gm('tests/spec/routes/205x50.png').toBuffer(function(err, buffer) {
		...
		it('should work when one of the numbers is larger than the image', function(done) {
			gm('tests/spec/routes/61x80.gif').toBuffer(function(err, buffer) {
				expect(err).toBeFalsy();
	...
	describe('get', function() {
		...
		beforeEach(function() {
			...
			User.findById
				.withArgs('abc')
				.yields(null, {avatar: 'tests/spec/routes/205x50.png'});
			User.findById
				.withArgs()
				.yields(null);
		});
